...
class HUD:
    ...
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        self._font_large = pygame.font.SysFont(
            "consolas", C.HUD_FONT_SIZE_LARGE, bold=True
        )
        ...
        self._ship_icon = pygame.transform.scale(self._make_mini_ship(), (28, 18))
    ...
    def update_high_score(self, score: int) -> None:
        ...
        self._high_score = max(self._high_score, score)
    ...
    def draw(self, player: PlayerShip, level: int) -> None:
        ...
        for i in range(player.lives):
            self._screen.blit(self._ship_icon, (m + 54 + i * 34, C.SCREEN_HEIGHT - 38))

        # --- Bottom neon dividers ---
        pygame.draw.line(self._screen, C.CYAN, (0, 65), (sw, 65), 1)
        pygame.draw.line(
            self._screen,
            C.GREEN,
            (0, C.SCREEN_HEIGHT - 48),
            (sw, C.SCREEN_HEIGHT - 48),
            1,
        )
    ...
    def draw_message(
        self, lines: list[str], colors: list[tuple], y_start: int = 0
    ) -> None:
        """
    ...
    def draw_score_popup(self, text: str, x: int, y: int, alpha: int = 255) -> None:
        """
